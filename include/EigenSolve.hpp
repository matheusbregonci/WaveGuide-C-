#pragma once

#include <algorithm>
#include <cmath>
#include <functional>
#include <random>
#include <vector>

// Generic lowest-eigenpair solver for a large symmetric positive-(semi)definite
// operator given only its matrix-free action y = A x. Uses subspace inverse
// iteration: a block of conjugate-gradient solves against A followed by
// Rayleigh-Ritz. Returns the `nModes` smallest eigenvalues and eigenvectors.
// Shared by the scalar Helmholtz and the vector Maxwell solvers.

namespace waveguide
{
    namespace eig
    {
        using ApplyOp = std::function<void(const std::vector<double>&,
                                           std::vector<double>&)>;

        inline double dot(const std::vector<double>& a, const std::vector<double>& b)
        {
            double s = 0.0;
            for (std::size_t i = 0; i < a.size(); ++i) s += a[i] * b[i];
            return s;
        }

        // Conjugate gradient: solve A x = b, x starts at 0.
        inline void cg(const ApplyOp& A, const std::vector<double>& b,
                       std::vector<double>& x, int maxIt, double tol)
        {
            const std::size_t N = b.size();
            x.assign(N, 0.0);
            std::vector<double> r = b, p = b, Ap(N);
            double rs = dot(r, r);
            const double rs0 = rs;
            if (rs0 == 0.0) return;
            for (int it = 0; it < maxIt; ++it)
            {
                A(p, Ap);
                const double denom = dot(p, Ap);
                if (denom <= 0.0) break;
                const double alpha = rs / denom;
                for (std::size_t i = 0; i < N; ++i) { x[i] += alpha * p[i]; r[i] -= alpha * Ap[i]; }
                const double rsN = dot(r, r);
                if (rsN <= tol * tol * rs0) break;
                const double beta = rsN / rs;
                for (std::size_t i = 0; i < N; ++i) p[i] = r[i] + beta * p[i];
                rs = rsN;
            }
        }

        inline void orthonormalize(std::vector<std::vector<double>>& X)
        {
            for (std::size_t j = 0; j < X.size(); ++j)
            {
                for (std::size_t k = 0; k < j; ++k)
                {
                    const double d = dot(X[j], X[k]);
                    for (std::size_t i = 0; i < X[j].size(); ++i) X[j][i] -= d * X[k][i];
                }
                double nrm = std::sqrt(dot(X[j], X[j]));
                if (nrm < 1e-300) nrm = 1.0;
                const double inv = 1.0 / nrm;
                for (double& v : X[j]) v *= inv;
            }
        }

        // Cyclic Jacobi eigensolver for a small symmetric dense p x p matrix
        // (row-major). Eigenvalues ascending; eigenvector j = V[i*p+j].
        inline void jacobiEig(std::vector<double> H, int p,
                              std::vector<double>& eval, std::vector<double>& V)
        {
            V.assign(std::size_t(p) * p, 0.0);
            for (int i = 0; i < p; ++i) V[i * p + i] = 1.0;
            for (int sweep = 0; sweep < 100; ++sweep)
            {
                double off = 0.0;
                for (int i = 0; i < p; ++i)
                    for (int j = i + 1; j < p; ++j) off += H[i * p + j] * H[i * p + j];
                if (off < 1e-26) break;
                for (int q = 0; q < p; ++q)
                    for (int r = q + 1; r < p; ++r)
                    {
                        const double apq = H[q * p + r];
                        if (std::abs(apq) < 1e-300) continue;
                        const double app = H[q * p + q], arr = H[r * p + r];
                        const double phi = 0.5 * std::atan2(2.0 * apq, arr - app);
                        const double c = std::cos(phi), s = std::sin(phi);
                        for (int k = 0; k < p; ++k)
                        {
                            const double hkq = H[k * p + q], hkr = H[k * p + r];
                            H[k * p + q] = c * hkq - s * hkr;
                            H[k * p + r] = s * hkq + c * hkr;
                        }
                        for (int k = 0; k < p; ++k)
                        {
                            const double hqk = H[q * p + k], hrk = H[r * p + k];
                            H[q * p + k] = c * hqk - s * hrk;
                            H[r * p + k] = s * hqk + c * hrk;
                        }
                        for (int k = 0; k < p; ++k)
                        {
                            const double vkq = V[k * p + q], vkr = V[k * p + r];
                            V[k * p + q] = c * vkq - s * vkr;
                            V[k * p + r] = s * vkq + c * vkr;
                        }
                    }
            }
            eval.resize(p);
            for (int i = 0; i < p; ++i) eval[i] = H[i * p + i];
            std::vector<int> ord(p);
            for (int i = 0; i < p; ++i) ord[i] = i;
            std::sort(ord.begin(), ord.end(), [&](int a, int b) { return eval[a] < eval[b]; });
            std::vector<double> ev(p), Vp(std::size_t(p) * p);
            for (int j = 0; j < p; ++j)
            {
                ev[j] = eval[ord[j]];
                for (int i = 0; i < p; ++i) Vp[i * p + j] = V[i * p + ord[j]];
            }
            eval = ev; V = Vp;
        }

        struct Result
        {
            std::vector<double> values;               // ascending
            std::vector<std::vector<double>> vectors;  // one per value
        };

        // Lowest `nModes` eigenpairs of A (SPD, size N), matrix-free.
        inline Result lowestEigenpairs(std::size_t N, const ApplyOp& A, int nModes,
                                       int cgIters = 400, int subIters = 40,
                                       int extra = 3)
        {
            Result out;
            if (N == 0) return out;
            const int p = std::min<int>(int(N), std::max(1, nModes) + extra);
            std::vector<std::vector<double>> X(p, std::vector<double>(N));
            std::mt19937 rng(20240714u);
            std::uniform_real_distribution<double> uni(-1.0, 1.0);
            for (int j = 0; j < p; ++j)
                for (std::size_t i = 0; i < N; ++i) X[j][i] = uni(rng);
            orthonormalize(X);

            std::vector<std::vector<double>> Y(p, std::vector<double>(N));
            std::vector<double> theta(p, 0.0), prev(p, 1e300);
            const double cgTol = 1e-6;
            for (int outer = 0; outer < subIters; ++outer)
            {
                for (int j = 0; j < p; ++j) cg(A, X[j], Y[j], cgIters, cgTol);
                orthonormalize(Y);
                std::vector<std::vector<double>> AY(p, std::vector<double>(N));
                for (int j = 0; j < p; ++j) A(Y[j], AY[j]);
                std::vector<double> H(std::size_t(p) * p, 0.0);
                for (int a = 0; a < p; ++a)
                    for (int b = a; b < p; ++b)
                    { const double h = dot(Y[a], AY[b]); H[a * p + b] = h; H[b * p + a] = h; }
                std::vector<double> ev, V;
                jacobiEig(H, p, ev, V);
                for (int j = 0; j < p; ++j)
                {
                    std::fill(X[j].begin(), X[j].end(), 0.0);
                    for (int k = 0; k < p; ++k)
                    {
                        const double s = V[k * p + j];
                        if (s == 0.0) continue;
                        for (std::size_t i = 0; i < N; ++i) X[j][i] += s * Y[k][i];
                    }
                }
                theta = ev;
                double maxrel = 0.0;
                const int want = std::min(p, std::max(1, nModes));
                for (int j = 0; j < want; ++j)
                    maxrel = std::max(maxrel, std::abs(theta[j] - prev[j]) / (std::abs(theta[j]) + 1e-30));
                prev = theta;
                if (outer > 2 && maxrel < 1e-6) break;
            }
            const int emit = std::min(p, std::max(1, nModes) + extra);
            out.values.assign(theta.begin(), theta.begin() + emit);
            out.vectors.assign(X.begin(), X.begin() + emit);
            return out;
        }
    } // namespace eig
} // namespace waveguide
